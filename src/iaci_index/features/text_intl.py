...
INPUT_FILE_LLM = "step5_A9_asean_features.xlsx"
OUTPUT_FILE_LLM = "step5_A10_tli_llm_features.xlsx"
...
INPUT_FILE_TLI = "step5_A9_asean_features.xlsx"
OUTPUT_FILE_TLI = "step5_A10_tli_features.xlsx"
...
def build_llm_prompt(school_name: str, context: Dict[str, Any]) -> str:
...
def kimi_fetch_tli_info(school_name: str, context: Dict[str, Any]) -> Dict[str, Any]:
    prompt = build_llm_prompt(school_name, context)
    client = _kimi_client()
...
def llm_complete_tli_features() -> None:
    print(">>> Step5-A10-LLM - 用大模型补全文本信息并计算 TLI 开始")

    df = pd.read_excel(INPUT_FILE_LLM)
    ...
    for idx, row in df.iterrows():
        ...
        if (idx + 1) % 20 == 0:
            print("  保存中间结果...")
            df.to_excel(OUTPUT_FILE_LLM, index=False)
    ...
    df.to_excel(OUTPUT_FILE_LLM, index=False)
    print(f"\n已保存带有 LLM 文本与 TLI 的表到：{OUTPUT_FILE_LLM}")
    print(">>> Step5-A10-LLM 完成")


def compute_text_intl_index() -> None:
    print(">>> Step5-A10 – 文本国际化指数 TLI 计算开始")

    df = pd.read_excel(INPUT_FILE_TLI)
    ...
    df.to_excel(OUTPUT_FILE_TLI, index=False)
    print(f"\n已保存：{OUTPUT_FILE_TLI}")
    print(">>> Step5-A10 完成")


def build_tli_features() -> None:
    """Run the TLI completion and scoring steps."""
    llm_complete_tli_features()
    compute_text_intl_index()
